...
function phraseState(page, resultsPerPage,phrasesCorrect,phrasesWrong,phrasesNotEnded, callback) {
    ...
    PhraseFacade.pagedList(page, resultsPerPage, function (err, phrases){
        if(err){
            ...
            phrases.forEach(function(phrase, index, phraseRef){
                        isEnded(phrase.Answers, function(resultEnded){
                            ...
                            /*if(phraseRef.length == 0) {
                                callback(0, phrasesNotEnded,phrasesCorrect,phrasesWrong);
                                return false;
                            }*/
                            // next page
                            if(index == phraseRef.length - 1 && phraseRef.length != 0){
                                phraseState(++page, resultsPerPage, phrasesCorrect,phrasesWrong,phrasesNotEnded, callback);
                            }
                            })
    ...
    callback(phrasesCorrect,phrasesWrong,phrasesNotEnded);
}  
...
var nc = [];
var  ne=[];
 phraseState(1,1,c,nc,ne, function(c,nc,ne){
         console.log("COMPLETE:")
    console.log(c);
    console.log("NON COMPLETE")
    console.log(nc)
    console.log("NON CONCLUSE")
    console.log(ne)
});
